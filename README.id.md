...
flowchart TD
    ...
    subgraph Fase 4: EKSEKUSI KODE
        ...
        Router -. Jalur Cepat Perbaikan Bug .-> BugAnalyst["Agent: sdlc-fix-bug-analyst\nSkill: sdlc-fix-bug"]
        BugAnalyst --> Engineer
    end
...
| :--- | :--- | :--- | :--- |
| **`sdlc-discovery-explorer`** | `sdlc-discover` | `sdlc-stay-on-rails` | **Discovery Report (Riset Awal)** |
...
| **`sdlc-solution-architect`** | `sdlc-design-spec` | `sdlc-model-threats`, `sdlc-design-ui`, `sdlc-plan-tests` | **Technical Specification (Spesifikasi Teknis)** |
...
| Peran Agent | Skill Utama | Skill Pendukung | Artefak Hasil Fase |
| :--- | :--- | :--- | :--- |
| **`sdlc-implementation-planner`** | `sdlc-plan` | `sdlc-plan-tests` | **Implementation Plan (Rencana Eksekusi)** |
| **`sdlc-traceability-auditor`** | `sdlc-audit-coverage` | - | **Laporan Audit Keterlacakan (Traceability)** |
...
| Peran Agent | Skill Utama | Skill Pendukung | Artefak Hasil Fase |
| :--- | :--- | :--- | :--- |
| **`sdlc-implementation-engineer`** | `sdlc-build` | `sdlc-stay-on-rails`, `sdlc-migrate-data`, `sdlc-audit-dependencies`, `sdlc-coordinate` | **Kode Sumber & Test Unit** |
| **`sdlc-fix-bug-analyst`** *(Jalur Bug)* | `sdlc-fix-bug` | `sdlc-plan-tests` | **Analisis Root Cause & Rencana Perbaikan** |
...
| Peran Agent | Skill Utama | Skill Pendukung | Artefak Hasil Fase |
| :--- | :--- | :--- | :--- |
| **`sdlc-code-reviewer`** | `sdlc-review-code` | `sdlc-audit-dependencies` | **Umpan Balik Review Kode** |
| **`sdlc-verification-engineer`** | `sdlc-verify` | `sdlc-plan-tests` | **Laporan Verifikasi Pengujian** |
...
| Peran Agent | Skill Utama | Skill Pendukung | Artefak Hasil Fase |
| :--- | :--- | :--- | :--- |
| **`sdlc-documentation-architect`** | `sdlc-document` | `sdlc-define-terms` | **Dokumentasi & Panduan Pengguna** |
| **`sdlc-release-engineer`** | `sdlc-ship` | `sdlc-coordinate` | **Kandidat Rilis Terverifikasi** |
| *(Operasional)* | `sdlc-add-observability` | `sdlc-handle-incident`, `sdlc-remember` | **Desain Log/Metrik & Incident Log** |
...
```text
"User melaporkan error 500 saat checkout ketika keranjang belanja kosong. Gunakan skill sdlc-fix-bug untuk melacak penyebab utama (root cause), buat test reproduksi, dan perbaiki dengan perubahan minimal."
```

### 3. Review Kode / Pull Request
```text
"Tolong lakukan review kode pada branch ini menggunakan skill sdlc-review-code. Periksa aspek keamanan, performa, dan kesesuaian dengan spesifikasi teknis kita."
```
...
```text
"Tolong audit repository ini menggunakan skill sdlc-ship sebelum kita melakukan rilis v1.0.0."
```
