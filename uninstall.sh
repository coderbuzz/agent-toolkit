...
if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/manifest.json" ] && [ -f "$SCRIPT_DIR/scripts/uninstall.sh" ]; then
    TOOLKIT_DIR="$SCRIPT_DIR"
else
    if ! command -v git >/dev/null 2>&1; then
        echo "Error: git is required for remote operation." >&2
        exit 1
    fi
...
if [ $# -eq 0 ]; then
    if [ -t 0 ] || [ -c /dev/tty ]; then
        if [ ! -t 0 ] && [ -c /dev/tty ]; then
            exec ./scripts/uninstall.sh < /dev/tty
        else
