...
(`skill_metadata.<skill>.persona_bound`). Persona-bound skills (e.g. `sdlc-review-code`,
`sdlc-design-spec`) carry a `## Dynamic Persona Activation` block in their `SKILL.md`.
...
to it, while utility skills (e.g. `sdlc-remember`, `sdlc-stay-on-rails`) can be invoked at any
